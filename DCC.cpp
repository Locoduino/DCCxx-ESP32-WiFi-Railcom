...
byte DCC::m_cut;         // 1/2 bit en cours d'envoi (pour la coupure de signal)
byte DCC::m_subBit;      // Partie du bit en cours d'envoi
byte DCC::m_dataMode;    // Variable d'état de l'automate paquet
byte DCC::m_packetUsed;  // Nombre de paquets à envoyer
byte DCC::m_headerCount; // Comptage des bits à un du préambule
byte DCC::m_bit;         // Bit en cours d'envoi
byte DCC::m_byteCount;   // Index de l'octet en cours d'envoi
byte DCC::m_bitShift;
byte DCC::m_cutoutCount; // Comptage de la séquence de coupure
long DCC::m_dccCount;
byte DCC::m_packetIndex;
byte DCC::m_packetSize[DCC_PACKET_NUM] = {0};
byte DCC::m_packetType[DCC_PACKET_NUM] = {0};
byte DCC::m_packetData[DCC_PACKET_NUM][DCC_PACKET_SIZE] = {{0}, {0}}; // Paquets de données à envoyer
...
void IRAM_ATTR DCC::interrupt(void)
{
